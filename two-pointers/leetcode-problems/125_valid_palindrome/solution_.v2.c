...
* 1. Count how many non-alphanumeric characters exist in the input string countNonAlphaNeu().
* 2. Create a temporary string that stores only alphanumeric characters getCleanAlphaNeumericString().
* 3. Convert all letters in the temporary string to lowercase getCleanAlphaNeumericString().
* 4. Use two pointers to check whether the temporary string is a palindrome isPalindrome().
*
* Improvement:
* - Created a function and seperated a resposibility out of isPalindrome() to make it more clean
*
* Author: F.C.Fahi 
* Date: 14 June 2026 
*/
...
void getCleanAlphaNeumericString(
    char *temp, 
    const char *s
) {
...
bool isPalindrome(
    ...
    getCleanAlphaNeumericString(temp, s);
